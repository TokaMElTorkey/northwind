...
function suppliers_insert(){
	...
	if(function_exists('suppliers_before_insert')){
		$args=array();
		if(!suppliers_before_insert($data, getMemberInfo(), $args)){ return false; }
	}
	...
	if(function_exists('suppliers_after_insert')){
		...
		$args=array();
		if(!suppliers_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}
...
function suppliers_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('suppliers');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='suppliers' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='suppliers' and pkValue='$selected_id'");
	...
	$rires = sql("select count(1) from `products` where `SupplierID`='".addslashes($SupplierID[0])."'", $eo);
...
function suppliers_update($selected_id){
	global $Translation;
	...
	if(function_exists('suppliers_before_update')){
		$args=array();
		if(!suppliers_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `suppliers` set       `CompanyName`=' . (($data['CompanyName'] !== '' && $data['CompanyName'] !== NULL) ? "'{$data['CompanyName']}'" : 'NULL') . ', `ContactName`=' . (($data['ContactName'] !== '' && $data['ContactName'] !== NULL) ? "'{$data['ContactName']}'" : 'NULL') . ', `ContactTitle`=' . (($data['ContactTitle'] !== '' && $data['ContactTitle'] !== NULL) ? "'{$data['ContactTitle']}'" : 'NULL') . ', `Address`=' . (($data['Address'] !== '' && $data['Address'] !== NULL) ? "'{$data['Address']}'" : 'NULL') . ', `City`=' . (($data['City'] !== '' && $data['City'] !== NULL) ? "'{$data['City']}'" : 'NULL') . ', `Region`=' . (($data['Region'] !== '' && $data['Region'] !== NULL) ? "'{$data['Region']}'" : 'NULL') . ', `PostalCode`=' . (($data['PostalCode'] !== '' && $data['PostalCode'] !== NULL) ? "'{$data['PostalCode']}'" : 'NULL') . ', `Country`=' . (($data['Country'] !== '' && $data['Country'] !== NULL) ? "'{$data['Country']}'" : 'NULL') . ', `Phone`=' . (($data['Phone'] !== '' && $data['Phone'] !== NULL) ? "'{$data['Phone']}'" : 'NULL') . ', `Fax`=' . (($data['Fax'] !== '' && $data['Fax'] !== NULL) ? "'{$data['Fax']}'" : 'NULL') . ', `HomePage`=' . (($data['HomePage'] !== '' && $data['HomePage'] !== NULL) ? "'{$data['HomePage']}'" : 'NULL') . " where `SupplierID`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function suppliers_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/suppliers_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/suppliers_templateDV.html');
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return suppliers_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
